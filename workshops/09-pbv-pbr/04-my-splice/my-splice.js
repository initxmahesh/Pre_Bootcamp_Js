...
// YOUR CODE BELOW
function mySplice(array, startIdx, deleteCount, string) {
  let removed = array.slice(startIdx, startIdx + deleteCount);
  before = array.slice(0, startIdx);
  after = array.slice(startIdx + deleteCount);
  let newArray;
  if (string !== undefined) {
    newArray = before.concat([string], after);
  } else {
    newArray = before.concat(after);
  }
  array.length = 0;
  array.push(...newArray);
